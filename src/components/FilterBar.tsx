    ...
    <div className="p-3 space-y-3 border-b border-gray-800">
      ...
      <div className="relative" ref={wrapperRef}>
        ...
        <input
          ...
          className="w-full bg-gray-800 border border-gray-700 rounded-lg pl-9 pr-8 py-2 text-sm text-white placeholder-gray-500 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none"
        />
        {query && (
          <button
            onClick={() => {
        ...
        {showDropdown && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-50 max-h-72 overflow-y-auto">
            ...
            {issueMatches.length > 0 && (
              <div>
                <p className="px-3 py-1.5 text-[10px] text-gray-500 uppercase tracking-wider border-b border-gray-800">
                  Issues
                </p>
                {issueMatches.map(issue => (
                  <button
                    ...
                    className="w-full flex items-start gap-2 px-3 py-2 text-left hover:bg-gray-800/60 transition-colors"
                  >
                    <FileText className="w-3.5 h-3.5 text-gray-500 mt-0.5 shrink-0" />
                    <div className="min-w-0">
                      <p className="text-sm text-gray-200 truncate">{issue.title}</p>
                      {issue.address && (
                        <p className="text-[11px] text-gray-500 truncate">{issue.address}</p>
                      )}
                    </div>
                  </button>
                ))}
      ...
      <div className="flex flex-wrap gap-2">
        <select
          ...
          className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-xs text-gray-300 outline-none focus:ring-1 focus:ring-emerald-500"
        >
